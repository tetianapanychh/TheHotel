(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function rotateText(element, rotationIncrement) {
            const text = element.innerText;
            element.innerHTML = [ ...text ].map(((char, i) => `<span style="transform:rotate(${i * rotationIncrement}deg)">${char}</span>`)).join("");
        }
        const facilitiesCircle = document.querySelector(".about-facilities__circular-text p");
        const aboutCircle = document.querySelector(".text-circular-about p");
        const galleryCircle = document.querySelector(".gallery__circular-text p");
        if (facilitiesCircle) rotateText(facilitiesCircle, 15);
        if (aboutCircle) rotateText(aboutCircle, 11);
        if (galleryCircle) rotateText(galleryCircle, 10.5);
        class Popup {
            constructor(options) {
                let config = {
                    logging: true,
        ...
        calendars.forEach((function(calendar) {
            const startElement = document.querySelector(calendar.start);
            const endElement = document.querySelector(calendar.end);
            if (startElement && endElement) {
                datepicker_min(startElement, {
                    id: calendar.id,
                    minDate: new Date,
                    maxDate: new Date(2025, 1, 0),
                    startDay: 1,
                    disableYearOverlay: true
                });
                if (endElement) {
                    datepicker_min(endElement, {
                        id: calendar.id,
                        disableYearOverlay: true,
                        startDay: 1
                    });
        ...
        function animOnScroll() {
            ...
            animItems.forEach((animItem => {
                ...
                const animItemOffset = animItemRect.top + pageOffset;
                const animItemHeight = animItemRect.height;
                const animItemPoint = windowHeight - animItemHeight / animStart;
                if (pageOffset > animItemOffset - animItemPoint && pageOffset < animItemOffset + animItemHeight) animItem.classList.add("_active"); else if (!animItem.classList.contains("anim-no-hide")) animItem.classList.remove("_active");
            }));
        }
